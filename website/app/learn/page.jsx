...
const SECTION_PATHS = {
  s1:  { zigzag: [0, -55, -82, -55,  0,  55, 82, 55],  interval: 180 },
  s2:  { zigzag: [72, -72,  72, -72, 72, -72, 72, -72], interval: 170 },
  s3:  { zigzag: [-78, -38, 12, 58, 80, 48, -8, -58],   interval: 185 },
  s4:  { zigzag: [0, -82, -65, -20, 30, 70, 82, 55],    interval: 175 },
  s5:  { zigzag: [60, 82,  55,  10, -38, -72, -80, -50], interval: 182 },
  s6:  { zigzag: [-35, 68, -80, 20, -58, 80, -15, 58],   interval: 178 },
  cap: { zigzag: [0],                                    interval: 180 },
}
...
export default function Learn() {
  ...
  function startItem(node) {
    ...
    if (node.isChapterTest) {
      ...
      const pool    = node.lessons?.length
        ? node.lessons.flatMap(l => l.questions ?? [])
        : [...node.questions]
      const hasFill = !!qData?.fillBlanks[node.id]
      const base    = fisherYates(pool).slice(0, hasFill ? 5 : 6)
